...
readonly LIB='./lib'
readonly PROGRAM="$1"
readonly PUBLIC_KEY_COMMON='ssh/id_rsa.common.pub'
...
ssh_append_authorized_keys() {

    func="ssh_append_authorized_keys"

    [ $DEBUG -eq 1 ] && echo $func

    test_file "$PUBLIC_KEY_COMMON"

    mkdir -p "${HOME}/.ssh"
    cat "$PUBLIC_KEY_COMMON" >> "${HOME}/.ssh/authorized_keys"

}

ssh_flash_config() {

    func="ssh_flash_config"

    [ $DEBUG -eq 1 ] && echo $func

    git clone "$REPO_SSH_CONFIG" "${GIT}/ssh-config"
...
main() {
    ...
    [ $PROGRAM = 0 ] && set_root_password

    [ $PROGRAM = 10 ] && from_pacman_install_packages
    [ $PROGRAM = 11 ] && from_flathub_install_packages
    [ $PROGRAM = 12 ] && from_repo_install_tailscale
    [ $PROGRAM = 13 ] && from_repo_install_yt_dlp
    [ $PROGRAM = 14 ] && from_web_get_emudeck_installer
    # [ $PROGRAM = 99 ] && from_web_install_discord         # NOTE: Deprecated in favor of flatpak version (just easier to maintain)

    [ $PROGRAM = 20 ] && ssh_append_authorized_keys
    # [ $PROGRAM = 21 ] && ssh_flash_config # TODO - wip

    [ $PROGRAM = 30 ] && install_steamos_tweak_io_scheduler  # still good as of steamos 3.7
    [ $PROGRAM = 31 ] && install_steamos_tweak_memory        # (same)
    [ $PROGRAM = 32 ] && install_steamos_tweak_watchdog      # (same)
